...
async def protection_handler(event):
    ...
    # Telefon Numarası Kontrolü (Her zaman aktif)
    phone_pattern = r'(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}'
    if re.search(phone_pattern, event.raw_text):
        await event.delete()
        return

    # Modlara Göre Silme İşlemi
    if bot_mode == 1:
        await event.delete()
    
    elif bot_mode == 2:
        if event.media:
            # Sesli mesaj (voice) veya ses dosyası (audio) değilse sil
...
if __name__ == '__main__':
    # Render ve Python 3.10+ için en sağlıklı döngü başlatma
    try:
